...
static func get_cylinder_boundings_box(p_pos: Vector3, p_cylinder: CylinderShape) -> AABB:
	return AABB(
		p_pos,
		Vector3(p_cylinder.get_radius(), p_cylinder.get_height() * 0.5, p_cylinder.get_radius())
	)

static func get_sphere_boundings_box(p_pos: Vector3, p_sphere: SphereShape) -> AABB:
