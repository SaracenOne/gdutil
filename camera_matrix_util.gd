...
func get_endpoints() -> PoolVector3Array:
	# Near Plane
	var near_plane : Plane = Plane(matrix[3] + matrix[2],
			matrix[7] + matrix[6],
			matrix[11] + matrix[10],
			-matrix[15] - matrix[14]).normalized()

	# Far Plane
	var far_plane : Plane = Plane(matrix[2] - matrix[3],
			matrix[6] - matrix[7],
			matrix[10] - matrix[11],
			matrix[15] - matrix[14]).normalized()
	...
	# Right Plane
	var right_plane : Plane = Plane(matrix[0] - matrix[3],
			matrix[4] - matrix[7],
			matrix[8] - matrix[11],
			- matrix[15] + matrix[12]).normalized()

	# Top Plane
	var top_plane : Plane = Plane(matrix[1] - matrix[3],
			matrix[5] - matrix[7],
			matrix[9] - matrix[11],
